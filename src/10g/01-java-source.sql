 ...
 *   2011-01-30: Added method setFsEncoding 
 *   2011-01-30: Done optimiztaions reg. Object allocation
 */
...
public class FileType implements SQLData {
  ...
  private static void doPrepareFileList(String pDir, boolean pRecursive) throws Exception {
    File[] poFileList = new File(pDir).listFiles();
 
    if (poFileList != null) {
      for (int i=0;i<poFileList.length;i++) {
        vLastDirListing.add(poFileList[i]);
        try {
          if (poFileList[i].isDirectory() && pRecursive) {
            doPrepareFileList(poFileList[i].getAbsolutePath(), true);
          }
  ...
  public static ARRAY readFiles(int pFileCount) throws Exception {
    ...
    if (iDirListCursor < vLastDirListing.size()) {
      ...
      while (iDirListCursor < vLastDirListing.size() && iCurrentFileCnt < pFileCount) {
        iDirListCursor++;
        iCurrentFileCnt++;
        vTransportFiles.add(convertToStruct((File)vLastDirListing.get(iDirListCursor - 1)));
      }
  ...
  private static void getFileList(String pDir, Vector oraFileEntries, boolean pRecursive) throws Exception {
    ...
    if (poFileList != null) {
      for (int i=0;i<poFileList.length;i++) {
        oraFileEntries.add(convertToStruct(poFileList[i]));
        try {
          if (poFileList[i].isDirectory() && pRecursive) {
            getFileList(poFileList[i].getAbsolutePath(), oraFileEntries, true);
          }
        } catch (java.security.AccessControlException e) {
        }
      }
  ...
  public BLOB getContentBLOB() throws Exception {
    ...
    BLOB fileContent = BLOB.createTemporary(FileType.con, true, BLOB.DURATION_CALL);
    bBuffer = new byte[fileContent.getChunkSize()];
  ...
  private void deleteRecursive(File currentDir) throws Exception {
    File[] poFileList = currentDir.listFiles();
    if (poFileList != null) {
      for (int i=0;i<poFileList.length;i++) {
        if (poFileList[i].isDirectory()) {
          deleteRecursive(poFileList[i]);
          poFileList[i].delete();
        } else {
          poFileList[i].delete();
        }
